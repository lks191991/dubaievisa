...
class SendEmailsNew extends Command
{
     ...
     * @var string
     */
    protected $signature = 'email:sendnew {schid}';
    ...
    public function handle()
    {
		...
		$schId = $this->argument('schid');
		 $schedule = ScheduleEmail::find($schId);
       ...
       // foreach ($schedules as $schedule) {
            if (!empty($schedule)) {
                ...
                $campaignEmails_total = CampaignEmail::where('campaign_id', $campaign_id)->with('emails')->get();

                $campaignTEmails_total = EmailTracker::where('campaign_id', $campaign_id)->get();
               /*  if($campaignTEmails_total->count() >= $campaignEmails_total->count())
                {
                    $schedule->status = ScheduleEmail::SENT;
                }
                $schedule->status = ScheduleEmail::SENT;
                $schedule->total_emails = $campaignEmails_total->count();
                $schedule->total_emails_send = $campaignTEmails_total->count();
                ...
                $bcc = Campaign::where('id', $campaign_id)->first()->bcc;
                $cc = Campaign::where('id', $campaign_id)->first()->cc;
                ...
                $campaignEmails = CampaignEmail::where('campaign_id', $campaign_id)->whereNull('sended_at')
                ->with('emails')->take(300)
                ->get();
                        ...
                        if (saas()) {
                            if (user_email_limit_check(trimDomain(full_domain())) == 'HAS-LIMIT') {
                               ...
                               // $campaign_email = new CampaignEmail();
                                $campaignEmail->sended_at = Carbon::now();
                                $campaignEmail->save();
                                ...
                                if ($campaign_attachment) {
                                    ...
                                    Mail::send('template_builder.template-detail', $data, function ($message) use ($bcc, $cc, $subject, $campaignEmail, $sender_email, $sender_name, $attachment) {
